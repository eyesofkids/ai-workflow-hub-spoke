---
name: wrap
description: 實作收尾與驗收前置檢查：自檢 test/lint/tsc/build 四綠、確認 report/runbook/issue_log 齊備、產出使用者手測清單與 diff 對照摘要、提示切 session（不 compact）。實作 session 完工或 context 吃緊時使用。
---
...
- `pnpm exec vitest run <本次異動模組的 *.test.ts>`（**只跑改到的**，判定範圍見 AGENTS.md 測試規範；禁止全站、禁止接 `| sort`/`| head`/`| tail`）
