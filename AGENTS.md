...
1. 規劃（hub，依必答檢查）→ plan 文件 → 使用者直接審定
   （可選：使用者點名找漏洞 spoke——sub-agent、need-to-know 工單、產出意見不產裁決；skill：/find-holes）
...
- **不用 compact**（有損，且 compact 後熱 session 價值已死）：context 吃緊 → 在最後一個自然斷點收尾——完工走 `/wrap`，**未完工走 `/wrap` 中途交接模式**（寫交接文 `handoff_<主題>.md`）→ 關 session → 新 session 依規劃書＋交接文冷啟動。發現對話開頭是 "This session is being continued" → 重讀規劃書，不信摘要裡的規格。
...
規劃書不送任何獨立 session 審查、不得建議恢復；歷史 `_docs` 中的「給審查閘的指引」不再適用。乾淨視角一律以「找漏洞 spoke」形式存在（見流程總覽第 1 步、`/find-holes` skill）。
